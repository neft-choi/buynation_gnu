...
$(function () {
  ...
  $(document).on("click", "#sit_sel_option li button", function () {
    ...
    if ($this.hasClass("sit_qty_plus")) {
      mode = "증가";
    } else if ($this.hasClass("sit_qty_minus")) {
      mode = "감소";
    } else if ($this.hasClass("sit_opt_del")) {
      mode = "삭제";
    } else {
      mode = $.trim($this.text());
    }
        ...
        if (confirm("선택하신 옵션항목을 삭제하시겠습니까?")) {
          ...
          if ($("#sit_sel_option .sit_spl_list").length > 0) {
            ...
            if ($el.hasClass("sit_opt_list")) {
              if ($(".sit_opt_list").length <= 1) del_exec = false;
            }
          }
