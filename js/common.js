...
// 반응형 스크립트
// <html> 태그에 적용된 CSS 변수 값을 읽어온다 (앞뒤로 공백 제거)
function getBreakpointValue(varName) {
  return getComputedStyle(document.documentElement).getPropertyValue(varName).trim();
}

// 가져온 변수 값과 조건이 현재 화면과 일치하는지 여부를 true/false 로 반환한다
function matchesBreakpoint(rule, varName) {
  return window.matchMedia(`(${rule}: ${getBreakpointValue(varName)})`).matches;
}
