...
// URL 쿼리스트링의 디버그 스위치를 확인한다. 예) ?debug=1
function g5_debug_flag($key = 'debug')
{
    return isset($_GET[$key]) && (string) $_GET[$key] === '1';
}

// SQL 실행 결과를 연관배열 목록으로 반환한다.
function g5_sql_fetch_all($sql, $error = G5_DISPLAY_SQL_ERROR, $link = null)
{
    $rows = array();
    $result = sql_query($sql, $error, $link);
