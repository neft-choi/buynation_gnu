...
<button type="button" class="cmt_btn">
    <span class="total !text-gray-900 !text-base">댓글 <?php echo $view['wr_comment']; ?>개</span>
...
<section id="bo_vc">
    ...
    ?>
        ...
        <article id="c_<?php echo $comment_id ?>" <?php if ($cmt_depth) { ?>style="margin-left:<?php echo $cmt_depth ?>px;border-top-color:#e0e0e0" <?php } ?> class="flex">
            ...
            <?php if ($is_comment_reply_edit) { ?>
                <div class="bo_vl_opt">
                    <button type="button" class="btn_cm_opt btn_b01 btn"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-ellipsis-vertical-icon lucide-ellipsis-vertical">
                        ...
                        </svg><span class="sound_only">댓글 옵션</span></button>
                    <ul class="bo_vc_act">
    ...
    <script>
        $(function() {
            // 댓글 옵션창 열기/토글
            $(document).off("click.cm_opt", ".btn_cm_opt").on("click.cm_opt", ".btn_cm_opt", function(e) {
                e.preventDefault();
                e.stopPropagation();

                var $menu = $(this).siblings(".bo_vc_act");
                $(".bo_vc_act").not($menu).hide();
                $menu.toggle();
            });

            // 댓글 옵션창 닫기 (옵션 영역 밖 클릭 시)
            $(document).off("mouseup.cm_opt").on("mouseup.cm_opt", function(e) {
                if ($(e.target).closest(".bo_vl_opt").length === 0) {
                    $(".bo_vc_act").hide();
                }
            });
        });
    ...
    <aside id="bo_vc_w" class="bo_vc_w">
        ...
        <form name="fviewcomment" id="fviewcomment" action="<?php echo $comment_action_url; ?>" onsubmit="return fviewcomment_submit(this);" method="post" autocomplete="off">
            ...
            <div class="flex gap-2">
                ...
                ?>
                <textarea id="wr_content" name="wr_content" maxlength="10000" required class="required !h-auto !border-gray-200  !bg-none" title="내용" placeholder="댓글내용을 입력해주세요"
            ...
            <div class="bo_vc_w_wr">
                ...
                <div class="btn_confirm">
                    ...
                    <button type="submit" id="btn_submit" class="btn_submit !bg-gray-900 !rounded-md !text-base font-medium">댓글등록</button>
                </div>
            </div>
        </form>
    </aside>
