...
<section class="mx-auto w-full max-w-full p-4">
    ...
    for ($i = 0; $row = sql_fetch_array($result); $i++) {
        ...
        $coupon .= '<li class="flex flex-col gap-2">' . PHP_EOL;
        $coupon .= '<div class="px-6 py-5 border border-yellow-400 bg-white rounded space-y-1">' . PHP_EOL;
        $coupon .= '<div class="coupon_name text-lg font-bold">' . $subj . '</div>' . PHP_EOL;
        $coupon .= '<div class="coupon_price text-base">' . $print_cp_price . ' 쿠폰</div>' . PHP_EOL;
        $coupon .= '<div class="coupon_meta_group">' . PHP_EOL;
        $coupon .= '<div class="coupon_content text-sm font-semibold">' . $cp_target . '</div>' . PHP_EOL;
        $coupon .= '<div class="text-sm text-zinc-400">적용 : ' . $cp_link . '</div>' . PHP_EOL;
        if ($row['cp_minimum']) {
            $coupon .= '<div class="text-sm text-zinc-400">최소주문금액 : <span class="cp_evt">' . number_format($row['cp_minimum']) . '원</span></div>' . PHP_EOL;
        }
        $coupon .= '</div>' . PHP_EOL;
        $coupon .= '<div class="text-sm">사용기한 : 다운로드 후 ' . number_format($row['cz_period']) . '일</div>' . PHP_EOL;
        // $coupon .= '<div class="mt-5 w-fitjjrounded-lg bg-orange-500 px-2 py-1 text-xs font-semibold text-white whitespace-nowrap">' . $cp_target . '</div>' . PHP_EOL;
        $coupon .= '</div>' . PHP_EOL;
        $coupon .= '<div class="coupon_btn"><button type="button" class="coupon_download h-10' . $disabled . '" data-cid="' . $row['cz_id'] . '" data-variant="primary">쿠폰다운로드</button></div>' . PHP_EOL;
        $coupon .= '</li>' . PHP_EOL;
    }

    if ($coupon)
        echo '<ul class="space-y-3">' . PHP_EOL . $coupon . '</ul>' . PHP_EOL;
    else
...
<section class="mx-auto w-full max-w-full p-4">
    ...
    for ($i = 0; $row = sql_fetch_array($result); $i++) {
        ...
        $coupon .= '<div class="text-sm">사용기한 : 다운로드 후 ' . number_format($row['cz_period']) . '일</div>' . PHP_EOL;
        $coupon .= '</div>' . PHP_EOL;
        $coupon .= '<div class="coupon_btn"><button type="button" class="coupon_download h-10' . $disabled . '" data-cid="' . $row['cz_id'] . '" data-variant="primary">포인트 ' . number_format($row['cz_point']) . '점 차감</button></div>' . PHP_EOL;
        $coupon .= '</li>' . PHP_EOL;
    }
...
<section class="couponzone_list !hidden">
    <h2>다운로드 쿠폰</h2>
...
<section class="couponzone_list !hidden" id="point_coupon">
    <h2>포인트 쿠폰</h2>
