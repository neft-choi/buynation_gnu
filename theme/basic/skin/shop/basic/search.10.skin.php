...
foreach ((array) $list as $row) {
    ...
    if ($i === 1) {
        ?>
        <ul class="sct grid grid-cols-2 pc:grid-cols-5 gap-x-2 pc:gap-x-5 gap-y-6 pc:gap-y-15">
            <?php
    }
    ?>
        <li class="sct_li js-search-recommend-item">
            <article class="space-y-2">
                ...
                <div class="flex w-full gap-1">
                    <button type="button"
                        ...
                        data-it_id="<?php echo $row['it_id']; ?>" aria-label="위시리스트">
                    ...
                    <button type="button"
                        ...
                        data-it_id="<?php echo $row['it_id']; ?>" aria-label="장바구니">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <?php if ($this->view_it_cust_price || $this->view_it_price) { ?>
                        <div class="flex flex-col">
                            <?php if ($this->view_it_cust_price && $it_cust_price > 0 && $it_cust_price > $it_price) { ?>
                                <span
                                    class="text-base text-gray-400 line-through"><?php echo display_price($it_cust_price, $row['it_tel_inq']); ?></span>
                            <?php } ?>
                            ...
                            <div class="flex gap-1 items-center">
                                ...
                                <?php if ($this->view_it_price) { ?>
                                    <span
                                        class="text-lg font-bold"><?php echo display_price(get_price($row), $row['it_tel_inq']); ?></span>
                                <?php } ?>
                            </div>
                            ...
                            <div class="flex gap-2 text-gray-900 text-sm">
                                <span class="flex gap-1 items-center">
                                    ...
                                    <?php echo $star_avg !== '' ? $star_avg : '0.0'; ?>
                                </span>
                                <span class="text-gray-300">|</span>
                                <span class="text-gray-400"><?php echo $use_cnt; ?>건</span>
                            </div>
                        </div>
                    <?php } ?>
...
<script>
    $(function () {
        syncWithPcBreakpoint(function (isPc) {
            const $items = $('#search_recommend .js-search-recommend-item');
