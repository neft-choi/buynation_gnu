                ...
                <form name="fsearch" method="get">
                    ...
                    <div class="relative">
                        <input type="text" name="stx" value="<?php echo stripslashes($stx); ?>" required id="stx"
                            class="h-12 w-full px-4 pr-14 rounded border border-zinc-300 text-base text-zinc-900 placeholder-zinc-400"
                            size="15" maxlength="15" placeholder="궁금하신 내용을 검색해보세요.">
                    ...
                    <div class="grid grid-cols-3">
                        ...
                        foreach ($qa_category_items as $index => $item) {
                            ...
                            <a href="<?php echo $item['href']; ?>"
                                class="flex items-center justify-center h-12 p-2 border-zinc-200 text-center text-sm <?php echo $right_border_class . $bottom_border_class . $active_class; ?>">
                                <?php echo $item['label']; ?>
                            </a>
                        <?php } ?>
                    </div>
            ...
            <div class="flex items-center justify-between gap-2">
                ...
                <?php if ($write_href) { ?>
                    <a href="<?php echo $write_href; ?>" title="문의하기"
                        class="px-4 py-2 rounded bg-gray-900 text-sm text-white">문의하기</a>
                <?php } ?>
            </div>

            <form name="fqalist" id="fqalist" action="./qadelete.php" onsubmit="return fqalist_submit(this);"
                method="post">
                            ...
                            <div class="<?php echo $lt_class ?> py-4 border-b border-slate-100 last:border-b-0">
                                ...
                                <div class="grid gap-2 text-sm text-slate-700">
                                    <div class="td_subject">
                                        <span
                                            class="bo_cate_link mr-2 text-xs text-slate-500 !hidden"><?php echo $list[$i]['category']; ?></span>
                                        <a href="<?php echo $list[$i]['view_href']; ?>"
                                            class="bo_tit inline-flex items-center gap-2 text-sm text-slate-900 !font-normal !break-all">
                                    ...
                                    <div class="flex items-center gap-4 text-sm text-slate-500">
                                        <span
                                            class="td_stat !w-fit text-xs <?php echo ($list[$i]['qa_status'] ? 'text-blue-400' : 'text-amber-600'); ?>"><?php echo ($list[$i]['qa_status'] ? '답변완료' : '답변대기'); ?></span>
                                        <div class="td_name_custom min-w-0 truncate text-left">
                                            <?php echo $list[$i]['name']; ?>
                                        </div>
                                        <div class="td_date_custom !w-fit text-right">
                                            <?php echo date('Y.m.d', strtotime($list[$i]['qa_datetime'])); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="bo_fx !my-2 !hidden">
                    <ul class="btn_bo_user">
                        <?php if ($is_checkbox) { ?>
                            <li><button type="submit" name="btn_submit" value="선택삭제" title="선택삭제"
                                    onclick="document.pressed=this.value" class="btn btn_b01 btn_admin"><svg
                        ...
                        <?php } ?>
                        <?php if ($list_href) { ?>
                            <li><a href="<?php echo $list_href ?>" class="btn_b01 btn" title="목록"><svg
                                        xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" aria-hidden="true">
                                        <path d="M8 6h13" />
                                        <path d="M8 12h13" />
                        ...
                        <?php if ($write_href) { ?>
                            <li><a href="<?php echo $write_href ?>" class="btn_b01 btn" title="문의등록"><svg
                                        xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2" aria-hidden="true">
                                        <path d="M12 20h9" />
