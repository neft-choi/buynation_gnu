...
if ($is_admin !== 'super' && !empty($member['mb_id'])) {
    $login_brand_sql = sql_real_escape_string(trim((string)$member['mb_id']));
    ...
    if (!empty($login_brand['brand_id'])) {
        $orderform_is_brand = true;
        $orderform_brand_id = trim((string)$login_brand['brand_id']);
        $orderform_brand_sql = sql_real_escape_string($orderform_brand_id);
    }
}
...
<section id="anc_sodr_list">
    ...
    <div class="local_desc02 local_desc">
        ...
        <?php if ($orderform_is_brand) { ?>
        <p>
            주문상품은 현재 로그인 브랜드
            <strong><?php echo get_text($orderform_brand_id); ?></strong>
            상품만 표시됩니다.
        </p>
        <?php } ?>
    </div>
