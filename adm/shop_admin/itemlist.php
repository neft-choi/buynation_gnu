...
<div class="md:bg-white bg-gray-50 mt-[-16px]">
    <div class="md:px-0 px-4 py-4 bg-white md:mx-0 mx-[-8px]">
        <?php echo $listall; ?>
    ...
    <form name="flist" class="local_sch01 local_sch !flex md:flex-row flex-col items-center gap-2 !m-0 p-4 md:border border-y border-gray-200 md:rounded rounded-none bg-white !mb-4  mx-[-8px]">
        ...
        <select name="sca" id="sca" class="w-full min-w-20 bg-gray-50">
        ...
        <div class="inline-flex w-full items-center gap-2">
            ...
            <select name="sfl" id="sfl" class="w-full min-w-20 bg-gray-50">
            ...
            <input type="text" name="stx" value="<?php echo $stx; ?>" id="stx" class="frm_input bg-gray-50">
        </div>

        <input type="submit" value="검색" class="md:w-fit w-full btn btn_04 ">
    </form>
    ...
    <form name="fitemlistupdate" method="post" action="./itemlistupdate.php" onsubmit="return fitemlist_submit(this);" autocomplete="off" id="fitemlistupdate">
        ...
        <div id="wide" class="tbl_head01 tbl_wrap">
            <table>
        ...
        <div id="compact" style="display:none;">
            <div class="text-xs text-gray-900 bg-white">
                <div class="flex items-baseline gap-2 p-4 border-t border-b border-gray-200">
                    <span class="text-sm font-black">상품목록</span>
                    <div>총 <span class="text-blue-500"><?php echo number_format($total_count); ?></span>건</div>
                </div>

                <?php
                $compact_result = sql_query($sql);
                for ($i = 0; $row = sql_fetch_array($compact_result); $i++) {
                    $href = shop_item_url($row['it_id']);
                ?>
                    <?= render_item_card($row, $i, array(
                        'qstr' => $qstr
                    )); ?>
                <?php
                }
                if ($i == 0) {
                    echo '<div class="p-6 text-center text-sm text-gray-500">자료가 한건도 없습니다.</div>';
                }
                ?>

                <?php echo render_item_card_menu_script('#compact'); ?>
            </div>
        </div>
...
<script>
    $(function() {
        // input, select 요소에 '수정'이 일어날 때만 실행
        $(".tbl_head01").on("input change", "input, select", function(e) {
            // 만약 클릭한 요소 자체가 체크박스(chk[])라면 자동 체크 로직을 실행하지 않음
            if ($(this).attr('name') === 'chk[]') return;

            var $tr = $(this).closest("tr");
            var $chk = $tr.find("input[name='chk[]']");

            // 현재 줄에 체크박스가 없다면(rowspan 구조), 위쪽 행에서 체크박스를 찾음
            if ($chk.length === 0) {
                $chk = $tr.prevAll().has("input[name='chk[]']").first().find("input[name='chk[]']");
            }

            // 값이 변경될 때만 체크 (이미 체크되어 있다면 건드리지 않음)
            if (!$chk.prop("checked")) {
...
<script>
    // 현재 뷰포트 너비가 768 이상인지
    // 객체가 반환 boolean은 mq.matches 로 확인
    ...
    function applyLayout() {
        // null 방어
        if (!wideBox || !compactBox) return;

        if (mq.matches) {
            wideBox.style.display = '';
            compactBox.style.display = 'none';
        } else {
            wideBox.style.display = 'none';
            compactBox.style.display = '';
        }
    }
