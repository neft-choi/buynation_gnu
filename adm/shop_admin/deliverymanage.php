            ...
            <section class="panel <?php echo $active_tab === 'individual' ? 'active' : ''; ?>" id="panel-individual">
                <div class="card" style="padding:20px">
                    <div class="card-head" style="padding:0 0 17px;min-height:auto;margin-bottom:15px">
                        <div>
                            <h2>그룹 미지정 상품</h2>
                            <p>이 상품들은 주문 시 배송비가 상품별로 합산됩니다.</p>
                        </div><button class="btn btn-small" id="openApplySecond" type="button" disabled>선택 상품 배송설정 (0)</button>
                    </div>
                        ...
                        <?php foreach ($ungrouped as $p) { ?>
                            <div class="product-row" tabindex="0" data-it-id="<?php echo get_text($p['it_id']); ?>" data-condition-id="<?php echo (int)$p['condition_id']; ?>">
                                ...
                                <div class="product-info">
                                    ...
                                    <div><strong><?php echo get_text($p['it_name']); ?></strong><small><?php echo get_text($p['it_id']); ?> · <?php echo get_text($p['dc_name']); ?> · 그룹 미지정</small></div>
                                </div>
                                <div class="product-actions">
                                    <div class="product-fee"><strong><?php echo delivery_product_fee_text($p); ?></strong><small>개별 부과</small></div><button class="btn btn-small row-shipping-btn" type="button">배송설정</button>
                                </div>
                            </div>
                        <?php } ?>
                    </div><?php if (!$ungrouped) { ?><div class="empty-products" style="display:block">그룹 미지정 상품이 없습니다.</div><?php } ?>
                </div>
            </section>
        </div>
    </main>
    ...
    <form id="fdeliverycondition" method="post" action="./deliverymanage_update.php"><input type="hidden" name="token" value="<?php echo get_text($admin_token); ?>"><input type="hidden" name="brand_id" value="<?php echo get_text($manage_brand_id); ?>"><input type="hidden" name="action" value="save_condition"><input type="hidden" name="dc_id" id="dc_id" value=""><input type="hidden" name="dc_type" id="dc_type" value="conditional">
        <div class="drawer-body">
            ...
            <div class="form-section">
                ...
                <div class="two-col">
                    <div><label class="form-label"><input type="checkbox" id="jejuUse" name="dc_jeju_use" value="1"> 제주 추가비 사용</label>
                        <div class="input-unit"><input class="form-control" id="jejuPrice" name="dc_jeju_price" type="number" min="0" value="3000"><span class="unit">원</span></div>
                    </div>
                    <div><label class="form-label"><input type="checkbox" id="islandUse" name="dc_island_use" value="1"> 도서산간 추가비 사용</label>
                        <div class="input-unit"><input class="form-control" id="islandPrice" name="dc_island_price" type="number" min="0" value="5000"><span class="unit">원</span></div>
                    </div>
                </div>
            </div>
...
<aside class="drawer" id="productGroupDrawer">
    <div class="drawer-head">
        <div>
            ...
            <p>선택 상품을 대상 그룹으로 이동합니다.</p>
        </div><button class="close-btn" type="button" data-close>×</button>
    </div>
    <form method="post" action="./deliverymanage_update.php" id="moveForm"><input type="hidden" name="token" value="<?php echo get_text($admin_token); ?>"><input type="hidden" name="brand_id" value="<?php echo get_text($manage_brand_id); ?>"><input type="hidden" name="action" value="move_products"><input type="hidden" name="group_id" id="moveGroupId">
        <div class="drawer-body">
            <div class="two-col select-block">
                <div><label class="form-label">상품 검색</label><input class="form-control" id="productPickSearch" type="search"></div>
                <div><label class="form-label">현재 소속</label><select class="form-control" id="productSourceFilter">
                        <option value="all">전체</option>
                        <option value="none">그룹 미지정</option><?php foreach ($groups as $g) { ?><option value="<?php echo (int)$g['dg_id']; ?>"><?php echo get_text($g['dg_name']); ?></option><?php } ?>
                    </select></div>
            </div>
            <div class="product-pick-list" id="productPickList"><?php foreach ($products as $p) { ?><label class="product-pick" data-name="<?php echo get_text($p['it_name']); ?>" data-no="<?php echo get_text($p['it_id']); ?>" data-group="<?php echo (int)$p['group_id']; ?>"><input type="checkbox" name="it_ids[]" value="<?php echo get_text($p['it_id']); ?>"><span class="product-thumb">▣</span><span class="product-pick-copy"><strong><?php echo get_text($p['it_name']); ?></strong><small><?php echo get_text($p['it_id']); ?> · <?php echo get_text($p['dc_name']); ?></small></span><span class="product-source <?php echo empty($p['group_id']) ? 'ungrouped' : ''; ?>"><?php echo empty($p['group_id']) ? '그룹 미지정' : get_text($p['dg_name']); ?></span></label><?php } ?></div>
        </div>
        <div class="drawer-foot"><button class="btn" type="button" data-close>취소</button><button class="btn btn-brand" type="submit">선택 상품 이동</button></div>
    </form>
</aside>
